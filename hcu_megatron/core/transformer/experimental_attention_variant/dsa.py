...
class DSAIndexer(MegatronModule):
    ...
    def forward_with_scores(
        ...
        """
        #assert packed_seq_params is None, "Packed sequence is not supported for DSAttention" #nhb
...
class DSAttention(MegatronModule):
    ...
    def forward(
        ...
        if packed_3d_input:
            ...
            # if x.dim() == 2:
            #     x, _ = _unpack_thd_to_sbh(x, cu_seqlens_q, packed_seq_params.max_seqlen_q)
            # if qr.dim() == 2:
            #     qr, _ = _unpack_thd_to_sbh(qr, cu_seqlens_q, packed_seq_params.max_seqlen_q)
